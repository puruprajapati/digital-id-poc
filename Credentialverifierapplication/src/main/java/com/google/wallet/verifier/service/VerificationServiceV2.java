...
public class VerificationServiceV2 {
  ...
  private List<CredentialDto.CredentialAttribute> parseMdocAttributes(byte[] mdocBytes, String credentialId) {
    ...
    try {
      ...
      if (documents.size() > 0) {
        ...
        // Get issuerSigned and issuerAuth for signature verification
        ...
        // VERIFY ISSUER SIGNATURE (Critical for production!)
        com.upokecenter.cbor.CBORObject issuerAuth = issuerSigned.get("issuerAuth");
        if (issuerAuth != null) {
          // Note: Full signature verification is complex and requires proper implementation
          // For now, we log the certificate info
          try {
            log.info("=== SIGNATURE VERIFICATION ===");
            log.info("Note: Signature verification is simplified in this POC");
            log.info("For production, implement full certificate chain validation");

            // Extract and log certificate info
            if (issuerAuth.getType() == com.upokecenter.cbor.CBORType.Array && issuerAuth.size() == 4) {
              com.upokecenter.cbor.CBORObject unprotectedHeaders = issuerAuth.get(1);
              com.upokecenter.cbor.CBORObject x5chain = unprotectedHeaders.get(com.upokecenter.cbor.CBORObject.FromObject(33));

              if (x5chain != null && x5chain.getType() == com.upokecenter.cbor.CBORType.Array && x5chain.size() > 0) {
                ...
                try {
                  ...
                  attributes.add(CredentialDto.CredentialAttribute.builder()
                    .name("issuer")
                    .value(cert.getIssuerX500Principal().getName())
                    .build());
          ...
          log.warn("No issuerAuth found - cannot verify signature!");
        }
